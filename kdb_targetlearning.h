...
class kdb_targetlearning :  public IncrementalLearner
{
public:
  kdb_targetlearning();
  kdb_targetlearning(char*const*& argv, char*const* end);
  ~kdb_targetlearning(void);
...
protected:
  ...
  void classify_local(const instance& inst,std::vector<std::vector<CategoricalAttribute> > parrent,std::vector<double> &posteriorDist);
  ...
  std::vector<std::vector<std::vector<CategoricalAttribute> > > local_parents;
  std::vector<std::vector<distributionTree> >local_dTrees;
